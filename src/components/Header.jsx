  ...
  return (
    <header className="lg:px-32  px-6 w-full">
      ...
      >
        ...
        <Typography color={theme.palette.colors.primary} fontSize={24} fontWeight={600}>
          BLOG SAVEEN
        </Typography>
        <ul className="md:flex  hidden space-x-10 text-xs">
          <li></li>
          {headers.map(item => <li><a href={item.to}>
        ...
        <div className="md:hidden self-start">
          ...
          <IconButton onClick={() => set_is_mobile_open(!is_mobile_open)} className="md:hidden self-start">
            {is_mobile_open ? <CloseIcon /> :
              <MenuIcon />
            }
          </IconButton>
        </div>
      ...
      {is_mobile_open &&
        <ul id="mobileNav" className="md:hidden space-x-10  duration-500  transition-all">
          <li></li>
          {headers.map(item => <li className="my-3"><a href={item.to}>
